...
class MatchingFlow {
  ...
  private:
    ...
    // publisher
    std::shared_ptr<CloudPublisher> global_map_pub_ptr_;    // 与local_map同等的分辨率
    std::shared_ptr<CloudPublisher> local_map_pub_ptr_;     // 与global_map同等的分辨率
    std::shared_ptr<CloudPublisher> current_scan_pub_ptr_;  // 原始分辨率的点云（与地图同等的分辨率）
    std::shared_ptr<OdometryPublisher> laser_odom_pub_ptr_; // 点云匹配得到的结果
