...
int main(int argc, char *argv[]) {
    ...
    std::shared_ptr<CloudSubscriber> cloud_sub_ptr = std::make_shared<CloudSubscriber>(nh, "/kitti/velo/pointcloud", 10000);
    std::shared_ptr<IMUSubscriber> imu_sub_ptr = std::make_shared<IMUSubscriber>(nh, "/kitti/oxts/imu", 10000);
    std::shared_ptr<GNSSSubscriber> gnss_sub_ptr = std::make_shared<GNSSSubscriber>(nh, "/kitti/oxts/gps/fix", 10000);
    std::shared_ptr<TFListener> lidar_to_imu_ptr = std::make_shared<TFListener>(nh, "velo_link", "imu_link");

    std::shared_ptr<CloudPublisher> cloud_pub_ptr = std::make_shared<CloudPublisher>(nh, "current_scan", 10000, "/map");
    std::shared_ptr<CloudPublisher> local_map_pub_ptr = std::make_shared<CloudPublisher>(nh, "local_map", 10000, "/map");
    std::shared_ptr<CloudPublisher> global_map_pub_ptr = std::make_shared<CloudPublisher>(nh, "global_map", 10000, "/map");
    // topic,base_frame,child_frame
    std::shared_ptr<OdometryPublisher> laser_odom_pub_ptr = std::make_shared<OdometryPublisher>(nh, "laser_odom", "map", "lidar", 100000); 
    std::shared_ptr<OdometryPublisher> gnss_pub_ptr = std::make_shared<OdometryPublisher>(nh, "gnss", "map", "lidar", 100000);
    ...
    Eigen::Matrix4f lidar_to_imu = Eigen::Matrix4f::Identity();   // Tf tree监听获得
        ...
        if (!transform_received) {
            ...
            while (cloud_data_buff.size() > 0 && imu_data_buff.size() > 0 && gnss_data_buff.size() > 0) {
                ...
                if (!time_inited) {
                    ...
                    run_time = cloud_data.time - init_time;     // 运行时间，实际上是rosbag播放点云数据的时间
                }
                ...
                if (d_time < -0.05) {
                    ...
                    gnss_data.UpdateXYZ();
                    odometry_matrix(0,3) = gnss_data.local_E;        // Tw_imu
                    ...
                    odometry_matrix *= lidar_to_imu;                 // Tw_lidar
                    gnss_pub_ptr->Publish(odometry_matrix);

                    if (!front_end_pose_inited) {
                        front_end_pose_inited = true;
                        front_end_ptr->SetInitPose(odometry_matrix); // 第一帧的位姿
                        LOG(INFO)<<"第一帧参考帧的坐标为："<<std::endl<<odometry_matrix<<std::endl;
                    }
                    // front_end_ptr->SetPredictPose(odometry_matrix); // 暂时没有卵用，可以用运动学估计
                ...
                if (run_time > 460.0 && !has_global_map_published) {
                    if (front_end_ptr->GetNewGlobalMap(global_map_ptr)) {
                        pcl::io::savePCDFileBinary("KITTI_MAP.pcd", *global_map_ptr);
                        LOG(INFO)<<"全局地图保存完成！"<<std::endl;
