...
bool FrontEnd::Update(const CloudData& cloud_data, Eigen::Matrix4f& cloud_pose) {
    ...
    // 匹配之后根据距离判断是否需要生成新的关键帧，如果需要，则更新局部地图
...
bool FrontEnd::UpdateNewFrame(const Frame& new_key_frame) {
    ...
    // 更新全局地图
    // 存储之前，点云先要释放
    // key_frame.cloud_data.cloud_ptr.reset(new CloudData::CLOUD()); // 在保存局部地图时候没有必要，但是在保存全局地图时候就会一直占用内存资源，但是这里不会对local_map的浅复制造成影响吗？会导致局部地图里只有一个关键帧吧
