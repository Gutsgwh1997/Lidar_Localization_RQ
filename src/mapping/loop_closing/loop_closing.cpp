 ...
 * @brief 初始化闭检测部分的参数值
 *
 * @param config_node loop_closing/config.yaml
 *
 * @return 
 */
...
/**
 * @brief 使用GNSS的位姿态从历史帧中检测最近的可能的回环帧
 *
 * @param key_frame_index 得到的可能的回环帧的索引
 *
 * @return 
 */
bool LoopClosing::DetectNearestKeyFrame(int& key_frame_index) {
    static int skip_cnt = 0;
    static int skip_num = loop_step_;              // 每隔10帧才做一次闭环检测
    if (++skip_cnt < skip_num)
        return false;

    if ((int)all_key_gnss_.size() < diff_num_ + 1) // 小范围的回环无效
        return false;
    ...
    if (min_distance > detect_area_) {  // 可以推测大于这一个距离，不会出现回环
        skip_num = std::max((int)(min_distance / 2.0), loop_step_);// 后端检测认为超过2m选取一个关键帧
        return false;
    } else {
        skip_num = loop_step_;
        return true;
...
/**
 * @brief 合成回环帧附近的局部地图
 *        需要注意的是把点云转换到GNSS参考系下       
 *        这样的局部地图更精确，因为GNSS比Lidar精确，这里接受的gnss早就在预处理中转换为lidar_to_world的了
 * @param key_frame_index 回环帧的索引
 * @param map_cloud_ptr 拼凑出来的局部地图
 * @param map_pose 局部地图的参考位姿
 *
 * @return 
 */
...
/**
 * @brief 构造当前帧的点云，主要是构造current_loop_pose_
 *
 * @param scan_cloud_ptr 当前帧的点云
 * @param scan_pose 当前帧点云的位姿
 *
 * @return 
 */
