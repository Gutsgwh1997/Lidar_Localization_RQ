...
VelocitySubcriber::VelocitySubcriber(ros::NodeHandle& nh, const std::string& topic_name, size_t buff_size)
    : nh_(nh) {
    subscriber_ = nh_.subscribe(topic_name, buff_size, &VelocitySubcriber::msg_callback, this);
}

void VelocitySubcriber::ParseData(std::deque<VelocityData>& velocity_data_buff) {
...
void VelocitySubcriber::msg_callback(const geometry_msgs::TwistStampedConstPtr& velocity_msg_ptr) {
