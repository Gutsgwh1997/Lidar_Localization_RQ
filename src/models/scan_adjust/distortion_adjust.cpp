...
bool DistortionAdjust::AdjustCloud(const CloudData::CLOUD_PTR& input_cloud_ptr, CloudData::CLOUD_PTR& output_cloud_ptr){
    ...
    float delete_space = 5.0 * M_PI / 180.0;    // 对于+-5度的区域不处理？？
    float start_orientation = atan2(origin_cloud_ptr->points[0].y, origin_cloud_ptr->points[0].x);

    Eigen::AngleAxisf t_V(start_orientation, Eigen::Vector3f::UnitZ()); // 雷达坐标系到第一帧的变换（旋转）
